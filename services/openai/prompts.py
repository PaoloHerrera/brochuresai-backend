class Prompts:
    def get_links_system_prompt(self) -> str:
        ...
        link_system_prompt += (
            "You should respond in JSON, but without the mark JSON as in this example:"
        )
        link_system_prompt += """
        {
            "links": [
    ...
    def brochure_system_prompt_professional(self, language: str) -> str:
        ...
        brochure_system_prompt += "Structure (landing page without explicit CTA): 1) Hero with company name and concise tagline; 2) What we do / Solutions (short overview); 3) Key benefits (bullet points); 4) Highlights/Features; 5) Clients or testimonials if available (brief); 6) About us & culture (optional and concise); 7) Footer with a discreet website link. If official social profiles exist, include a small ‘Follow us’ block with clickable links (icons + text).\n"
    ...
    def brochure_system_prompt_funny(self, language: str) -> str:
        ...
        brochure_system_prompt += "Structure: Hero (name + playful tagline); What we do / Solutions; Key benefits (bullets); Highlights/Features; Clients/testimonials if available; About us/culture (optional, brief); Footer with a discreet website link plus a cute ‘Follow us’ row if social links exist.\n"
